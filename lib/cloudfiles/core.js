...
Cloudfiles.prototype.copyFile = function (originContainer, originPath, destContainer, destPath, callback) {
    var copyOptions = {
      method: 'COPY',
      uri: this.storageUrl(originContainer, originPath),
      headers: {
        'DESTINATION': '/' + [destContainer, destPath].join('/'),
      },
      client: this
    };
    
    common.rackspace(copyOptions, callback, function (body, res) {
      console.log('COPY RESPONSE: ' + body, res);
      callback(null, true);
    });
};
